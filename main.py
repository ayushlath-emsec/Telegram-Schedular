    ...
    if (isNodeBusy!=True):
        if collection.count_documents({'isUrgent':True})>0:
            ...
            if collection.count_documents({"status":{"$ne":"running"},"time":{"$lte":d}})>0:
                #print(f"No of Channel whose status not running: {collection.count_documents({'status':{'$ne':'running'},'time':{'$lte':d}})}")
...
async def main():
    ...
    sched.add_job(telegram_scrapping,'interval',seconds=45)
    sched.start()
